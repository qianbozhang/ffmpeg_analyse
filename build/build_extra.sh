...
#ffmpeg
mkdir -p $BUILD_TEMP/ffmpeg-4.2.2
cd $BUILD_TEMP/ffmpeg-4.2.2
if [ ! -f "build_ok" ]; then
    echo ">>>>>>>>>>>>>>>>>>>>>>>>>>> build ffmpeg-4.2.2 >>>>>>>>>>>>>>>>>>>>>>>";
    CC=$CC $EXTRA_DIR/ffmpeg-4.2.2/configure --prefix=$INSTALL_DIR \
        --enable-shared
    make -j 8 && make install
    if [ $? -eq 0 ]; then
        touch build_ok;
        echo "ffmpeg-4.2.2 compiled successfully, please delete \"build_ok\" file in $BUILD_TEMP/ffmpeg-4.2.2 if you wanted rebuild.";
    else
        echo "ffmpeg-4.2.2 compiled fail, please fixed error and retry.";
        exit 0;
    fi;
else
    echo "ffmpeg-4.2.2 has been compiled.";
fi;